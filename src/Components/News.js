...
const News = (props) => {
    ...
    const updateNews= async() => {
        document.title = `News Panda | ${props.category[0].toUpperCase()+props.category.slice(1)}`;
    ...
    const fetchMoreData = async () =>{
        ...
        return (
            <div className="container my-3">
                <h2 style={{backgroundColor:'#FFD0EC',borderRadius:'50px',justifyContent:'center',textAlign:'center',color:'#1F2544'}}> {props.category[0].toUpperCase() + props.category.slice(1)} - Latest Headlines</h2>
                {loading && <Loading />}
                <InfiniteScroll
